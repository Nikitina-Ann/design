...
public class ManagerFrame extends JFrame {
    ...
    public ManagerFrame(int id) {
        ...
        nameField.setText("Здравствуйте, " + manager.getName());
        showTables();
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    if (tab == 0) {
                        ...
                        if (!SpecialOfferService.setClientId(specialOffers.get(selectIndex), clientName)) {
                        ...
                        SpecialOfferService.setAgreement(SpecialOffer.getApprovedSpecOffer(approvedSpecialOffers, selectIndex));
                    } else if (tab == 2) {
                        selectIndex = table3.getSelectedRow();
                        String financierName = roleField.getSelectedItem().toString();
                        BidService.setFinancierId(Bid.getBidForBidsManager(bids, selectIndex), financierName);
                    } else if (tab == 3) {
                        selectIndex = table4.getSelectedRow();
                        BidService.setAgreement(Bid.getBidForApprovedBidsForManager(bids, selectIndex));
                    } else if (tab == 7) {
                        selectIndex = table8.getSelectedRow();
                        RestrBidService.setAgreement(RestrBid.getBidForApprovedRestrBidsForManager(restrBids, selectIndex));
                    } else if (tab == 6) {
                        selectIndex = table7.getSelectedRow();
                        String financierName = roleField.getSelectedItem().toString();
                        RestrBidService.setFinancierId(RestrBid.getBidForRestrBids(restrBids, selectIndex), financierName);
                    }
                } catch (Exception nfe) {
    ...
    public void showTables() {
        ...
        restrBids = RestrBidService.getBidByRoleId(id, 2);
        ...
        String[][] data1 = SpecialOffer.showTable(specialOffers);
        String[][] data2 = SpecialOffer.showApprovedTable(approvedSpecialOffers);
        String[][] data3 = Bid.showBids(bids, true);
        String[][] data4 = Bid.showApprovedBidsForManager(bids);
        String[][] data5 = Bid.showAgreements(bids, true);
        String[][] data6 = SpecialOffer.showAgreements(approvedSpecialOffers, true);
        String[][] data7 = RestrBid.showRestrBids(restrBids, true);
        String[][] data8 = RestrBid.showApprovedRestrBids(restrBids, true);
