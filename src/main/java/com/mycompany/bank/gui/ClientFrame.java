...
public class ClientFrame extends javax.swing.JFrame {
    ...
    public ClientFrame(int id) {
        ...
        nameField.setText("Здравствуйте, " + client.getName());
        CreateMenu();
        ShowBid();
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    if (tab == 0) {
                        ...
                        SpecialOfferService.setResponseClient(client, SpecialOffer.getNotAgreement(client.getSpecialOffers(), selectIndex), answer);
                    } else if (tab == 2) {
                        Boolean answer = Boolean.parseBoolean(buttonGroup.getSelection().getActionCommand());
                        selectIndex = table3.getSelectedRow();
                        BidService.setResponseClient(Bid.getBidForApprovedBidsForClient(bids, selectIndex), answer);
                    } else if (tab == 3) {
                        selectIndex = table4.getSelectedRow();
                        String reason = getReasonField.getText();
                        RestrBidService.createNewBid(bids.get(selectIndex), reason);
                    } else if (tab == 6) {
                        Boolean answer = Boolean.parseBoolean(buttonGroup.getSelection().getActionCommand());
                        selectIndex = table7.getSelectedRow();
                        RestrBidService.setResponseClient(RestrBid.getBidForApprovedRestrBidsForClient(restrBids, selectIndex), answer);
                    }
    ...
    public void ShowBid() {
        ...
        String[][] data1 = SpecialOffer.showTable(client.getSpecialOffers());
        String[][] data2 = Bid.showBids(bids, false);
        String[][] data3 = Bid.showApprovedBidsForClient(bids);
        String[][] data4 = Bid.showAgreements(bids, false);
        String[][] data5 = SpecialOffer.showAgreements(client.getSpecialOffers(), false);
        String[][] data6 = RestrBid.showRestrBids(restrBids, false);
        String[][] data7 = RestrBid.showApprovedRestrBids(restrBids, false);
